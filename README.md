...
| K210 | Kendryte SDK 固件 | 自带摄像头：JPEG 预览流**常开**（与推理解耦）+ 指令触发的 KPU 车牌识别 |
...
- C（core0_service / m4_fw）、C++ Qt5（core1_ui 界面）、K210 固件（Kendryte SDK / MaixPy）
...
├── k210_fw/                    # K210 固件（Kendryte SDK 工程）
│   ├── capture/                # DVP 摄像头采集、JPEG 编码
│   ├── preview/                # 预览流持续发送（与推理业务解耦）
│   ├── infer/                  # KPU 车牌识别（指令触发）
│   └── uart_proto/             # 串口协议：分帧、指令解析、JSON 结果
...
│   └── CMakeLists.txt
├── docs/
│   └── protocols.md            # 接口规格：A7↔M4 帧、A7↔K210 串口帧与 JSON、
│                               # Modbus 寄存器表、双 A7 共享结构体（待建，随代码维护）
└── deploy/
...
2. **K210 固件**（`k210_fw/`）：Kendryte SDK 交叉编译 → 烧录进 K210 flash；上电即常开输出 JPEG 预览流（接线与帧协议见 `Task.md` 第五节 / `docs/protocols.md`）。
...
两份文档内容不重复；交叉处只做链接引用，字段级协议规格统一归属 `docs/protocols.md`（待建）。
