...
- 字节序：RPMSG/K210 多字节字段小端；**CAN 帧多字节字段大端**（见 §1）。
...
## 1. CAN 总线（C8T6 ↔ M4）—— **接口 v2**（2026-09-11 定版；v1 的"lux/drop% 原始值上报"已废弃）
...
- **多字节字段一律大端**（CAN 章自定；RPMSG/K210 侧小端，跨通道由 M4 转换，见 §3）。

### 1.0 ⭐ 设备抽象与传感器隔离原则（v2 的立身之本）

1. **总线上只有语义**：CAN 帧不出现 lux、drop%、ADC 值、PWM 脉宽等任何传感器/执行器原始量；只出现"事件码 + 语义参数 + 状态位 + 节点 tick"。
2. **节点自治**：采样方式、阈值、去抖全属下位机私有实现；板级(M4/A7)对"下位机用了哪种传感器"**零假设**。
3. **换传感器不动 Linux**：换传感器只改下位机固件 + `c8t6/Core/Inc/config.h` 的映射表；`m4_fw`/`core0_service`/`core1_ui` 一行不改。
4. **语义档位代替数值下发**：板级只发"灵敏度档位 1..5"（0x100/0x10），由节点自己翻译成物理阈值。
5. **未知即忽略**：不认识的事件码/指令码只计数不报错（前向兼容：加新事件不用改老接收方）。
6. **节点身份随帧**：0x210 带 `dev_type`/`fw_ver`/`node_id`，主端据此识别"谁在说话、什么版本"。
7. **事件码单一事实源**：CAN 0x200 的 `d[0]` 与 RPMSG 0x21 的 `code` **同一张表**（下表），跨通道不重编号。

### 0x100 主→从 指令帧（DLC=8，未用字节 0）

| 字节 | 含义 |
|---|---|
| d[0] | cmd：0x01 开闸 ｜ 0x02 关闸 ｜ 0x03 状态查询（从端**立即**回一帧 0x210）｜ 0x10 设置检测灵敏度档位 |
| d[1] | arg：cmd 的语义参数（0x10 时 = 档位 1..5；其余 = 0） |
| d[2] | seq：指令序号（主端自增、按 u8 回绕；**发送失败不消耗**）。当前节点**不对 0x100 单独回执**——指令是否真生效由 0x200/0x04 闸位事件 + 0x210 状态位体现；seq 保留给将来多节点/重传对账 |
| d[3..7] | 0 |

### 0x110 主→从 回执帧（DLC=8；**只有主端发、从端只收**）
...
- 主端(M4)每收到一帧 0x200 自动回 `0x110/kind=0x01`。从端(C8T6)收到后 OLED 行4 显示 `CAN:Sended`（约 OLED_ACK_FLASH_MS=1s）。
- ⚠️ **从端不得发 0x110**：它在 0x1xx（主→从）段，而主端 FDCAN 过滤器只收 0x200~0x2FF，从端发出去没有任何接收者（v2 定稿时踩过：曾让 C8T6 回"指令确认"，属无效流量，已删）。
...
| d[1..2] | arg（大端 u16）：0x01/0x02/0x05 = 0；0x03 = 故障类别（1 传感器 / 2 执行器 / 3 CAN）；0x04 = 闸位（0 关 / 1 开；2 = 动作中，预留未用） |
| d[3] | 状态位：bit0 闸开 ｜ bit1 检测区有车/物体 ｜ bit2 传感器故障 ｜ bit3 CAN 错误（bit4..7 预留） |
| d[4..7] | tick（大端 u32）：**节点**上电毫秒（49.7 天回绕），供主端/A7 做时序对齐 |

- 事件是**边沿通知**：0x03 只在故障"发生"时发一次（当前是否仍有故障以状态位 bit2 为准）；0x04 只在闸**执行到位**翻转时发（缓动途中不发）；0x05 在首次心跳成功后发一次。

### 0x210 从→主 心跳帧（DLC=8，1Hz；同时是 0x100/0x03 的查询应答）

| 字节 | 含义 |
|---|---|
| d[0] | 同 0x200 d[3] 状态位 |
| d[1..4] | uptime_ms（大端 u32）：节点上电毫秒 |
| d[5] | dev_type：0x01 = 存在检测(光感) + SG90 道闸节点 |
| d[6] | fw_ver：高 4 位主版本 / 低 4 位次版本（当前 0x20 = v2.0） |
| d[7] | node_id：节点号（当前 0x01 = 主检测节点） |

- 超时：主端 3s 无 0x210 → 判从节点离线并上报 A7；从端不判主端。
- **档位 1..5 的物理含义由节点私有定义**（当前 C8T6：档位 3 = 掉点阈值 60%，见 `c8t6/can.md`）——协议只保证"档位越高越灵敏"这一个语义约定。
...
|---|---|---|
| 0x21 | 节点语义事件（v2） | 共 9B：`code[0]=事件码` ｜ `arg[1..2]=语义参数 u16 LE` ｜ `status[3]=状态位` ｜ `node_id[4]=节点号` ｜ `tick[5..8]=M4 接收时刻毫秒 u32 LE` |
...
- 0x21 的语义字段 = §1 的 0x200 事件（`code`/`arg`/`status` 与 CAN 逐字段对应，**事件码是同一张表**）；字节序在 M4 由大端(CAN)换成小端(RPMSG)。**CAN 原始帧不再透传**——lux/drop% 一类传感器数值到此为止，不下床、不进 A7（§1.0）。
- 帧长兼容：v2 的 0x21 固定 9B；Core0 收到 `len=17` 的 0x21（v1 原始透传）会明确报 `0x21 decode failed: len=17 (v1 CAN passthrough? reflash M4 with interface v2)`。
...
| 通道 | 内容概要 | 状态 |
|---|---|---|
| ~~Modbus-TCP 寄存器表（Core0 ↔ 上位机）~~ | 原 0x0000~0x0041 寄存器映射 | **已取消（2026-09-11）**：Demo 无 PC 上位机；监控归 Core1 LCD 读共享内存、远程控制走 0x11/0x12、白名单改本地配置热加载。寄存器表已从本文件删除 |
...
| 日期 | 变更 | 影响端 |
|---|---|---|
| 2026-09-11 | **接口 v2 修订（同日，评审发现）**：原稿让 C8T6 用 0x110 回"指令确认"——方向错误（0x110 属 0x1xx 主→从段，主端过滤器只收 0x200~0x2FF，从端发出去无人接收），已删除该回执与 `CAN_ACK_KIND_CMD`，0x110 只保留 `kind=0x01` 事件确认；0x100 的 seq 保留但当前不回执（效果确认走 0x200/0x04 闸位事件 + 状态位） | C8T6 / M4 |
