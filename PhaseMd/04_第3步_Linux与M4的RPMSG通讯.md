...
- [x] P3-01/05 remoteproc 加载稳定、ttyRPMSG0 出现 ttyRPMSG0 就位、creating channel，2026-09-10 起稳定
- [x] P3-07 帧协议 10 分钟无解析错 2026-09-10 python 验收 CRC 0 错；未单独计时 10 分钟
- [x] P3-08 断链 ≤1s 检出 沿用 2026-09-10 结论（1s 判定未改）
- [x] P3-09 恢复重同步、无旧 seq 污染 沿用 2026-09-10 结论（重连清 seq 未改）
- [x] P3-02/03 事件上行 + 指令下行端到端（Linux 发指令 → SG90 动作） 2026-09-12 v2 板验：事件上行 + 指令下行 → SG90 均通
- [ ] P3-04 CAN 离线/恢复经 RPMSG 可见 未专项验：见 m4_fw/rpmsg.md §5（拔 C8T6 测 0x22）
- [x] P3-11 CLI 工具入库 rpmsg_cli 源码已入库；未交叉编译（板上未跑）
