...
- [ ] 手遮 BH1750（档位 3 默认：掉点 ≥60% 连续 3 次 ≈ 0.6s）→ 行2 drop% 增大、检出翻转，行4 闪 `CAN:EVT`（本端 0x200/0x01 已发出，但**帧里没有 lux**）；移开恢复再闪一次（0x200/0x02）。
- [ ] 有逻辑分析仪/USB-CAN 工具时：抓 0x200/0x210 帧，字段与 §4 表比对（arg/tick **大端**！0x210 的 d[5..7] = dev_type/fw_ver/node_id）。
- [ ] 用上位机/CAN 工具发 `0x100 data=10 05 01 ...`（档位 5）→ 再手遮，翻转应更灵敏；发 `10 01 01` 应回到最不灵敏。
