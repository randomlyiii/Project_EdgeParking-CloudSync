 ...
 *   - 500k: PLL3=100MHz / Prescaler=40 / (1+Seg1+Seg2)=5 → bit=2µs；采样点
 *     (1+3)/5 = 80%（原 Seg1=2/Seg2=2 是 60%，与从端 75% 差太远，故改 3/1）。
 *   - TxFifoQueueElmtsNbr 必须 >0，否则 HAL_FDCAN_AddMessageToTxFifoQ 无从发送。
 *   CubeMX 若再生成 FDCAN1 需确认这两处未被回退。 */
/* USER CODE END 0 */
...
void MX_FDCAN1_Init(void)
{
  ...
  hfdcan1.Init.NominalTimeSeg1 = 3;   /* 采样点 80%(与 c8t6 从端 75% 匹配, 见文件头注) */
  ...
  hfdcan1.Init.TxFifoQueueElmtsNbr = 8;   /* 指令发送经 TX FIFO, 需 >0(见文件头注) */
